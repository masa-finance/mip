...
The scope of work for this proposal is to move the scraping code from the oracle and subnet repositories into a new repository, which will be responsible for scraping the data and encrypting it. The new repository will be called "masa-tee-scraper".

The masa-tee-scraper will provide an API which can be used by the oracle to scrape the data and decrypt it. The Oracle code will have to be modified to be a client for the masa-tee-scraper API. The masa-tee-scraper then will run in an enclave and will encrypt the data before sending it back to the oracle or the subnet. The consumers of the data (e.g. who is finally reading its content) will have to decrypt-it by using its local instance of the masa-tee-scraper.
